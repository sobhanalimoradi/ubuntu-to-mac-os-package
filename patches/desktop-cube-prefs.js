...
export default class DesktopCubePreferences extends ExtensionPreferences {
  ...
  fillPreferencesWindow(window) {
    ...
    // The cube needs a fixed (non-dynamic) workspace count to render
    // correctly, and GNOME buries that control elsewhere. Expose it here
    // directly, bound to the real system settings (not this extension's
    // own schema).
    ...
    const workspaceGroup = new Adw.PreferencesGroup({
      title: 'Workspaces',
      description: 'The cube requires a fixed workspace count. If dynamic ' +
        'workspaces is on, disable it here to set an exact number.',
    });

    const dynamicRow = new Adw.SwitchRow({title: 'Dynamic Workspaces'});
    mutterSettings.bind(
      'dynamic-workspaces', dynamicRow, 'active', Gio.SettingsBindFlags.DEFAULT);
    workspaceGroup.add(dynamicRow);

    const countRow = new Adw.SpinRow({
      title: 'Number of Workspaces',
      adjustment: new Gtk.Adjustment(
        {lower: 1, upper: 100, step_increment: 1, page_increment: 4}),
    });
    wmSettings.bind(
      'num-workspaces', countRow, 'value', Gio.SettingsBindFlags.DEFAULT);
    workspaceGroup.add(countRow);

    this._pages[0].add(workspaceGroup);
