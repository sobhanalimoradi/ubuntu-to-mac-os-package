  ...
  compiz_getIcon(actor) {
      ...
      dock.dash._box
        ...
        .forEach((dashElement) => {
          ...
          if (pids && pids.indexOf(pid) >= 0) {
            ...
            let transformed_size = dashElement.get_transformed_size();
            // dock._iconSizeScaledDown is the authoritative current icon
            // size (same value used to size the renderer actor itself);
            // prefer it over transformed_size, which can be stale/zero
            // during hover-magnify or other in-flight transforms.
            let realIconSize = dock._iconSizeScaledDown
              ? dock._iconSizeScaledDown * (dock.getMonitor().geometry_scale || 1)
              : null;
            if (
              ...
              dashIcon = {
                ...
                width: realIconSize || (transformed_size ? transformed_size[0] : 0),
                height: realIconSize || (transformed_size ? transformed_size[1] : 0),
              };
              return;
            }
          }
    ...
    switch (dock._position) {
      ...
      case 'top':
        ...
        break;
      case 'bottom':
        // Target the icon's actual current position (which already
        // reflects autohide state) instead of forcing the animation to
        // always fly to the literal bottom edge of the monitor.
        y += h / 2;
        x += w / 2;
        break;
    }
