...
export class WobblyEffect extends Clutter.DeformEffect {
    ...
    vfunc_set_actor(actor) {
        ...
        if (actor && !this.initialized) {
            ...
            // The rounded-corners shader clips to the window's undeformed
            // rectangle, which cuts off the wobble past that boundary.
            // Disable it for the duration of the wobble animation.
            let roundedCornersEffect = actor.get_effect('Rounded Corners Effect');
            if (roundedCornersEffect) {
                roundedCornersEffect.set_enabled(false);
            }
        }
    }
    ...
    vfunc_modify_paint_volume(pv) {
        ...
        let paddingX = actor.width * 0.3;
        let paddingY = actor.height * 0.3;
        ...
        pv.set_width(pv.get_width() + paddingX * 2);
        pv.set_height(pv.get_height() + paddingY * 2);
    ...
    destroy() {
        ...
        if (actor) {
            ...
            let roundedCornersEffect = actor.get_effect('Rounded Corners Effect');
            if (roundedCornersEffect) {
                roundedCornersEffect.set_enabled(true);
            }
